...
class OptimalController:
    """Beta-scheduled rival continuation with late ambiguity repair."""
    ...
    def solve(self, env: FlowTTSEnv, beta: float) -> AnswerRecord:
        ...
        try:
            ...
            finalists = self._late_repair_stage(env, finalists, schedule, initial_budget)
    ...
    def _schedule(self, beta: float, budget: int) -> dict[str, float | int | str]:
        ...
        if beta <= 0.0:
            return {
                ...
                "late_repair_time": 0.0,
                "late_repair_eval_time": 0.0,
                "keep_after_scout": 1,
                "keep_after_commit": 1,
                "scout_margin": 0.05,
                "commit_margin": 0.04,
                ...
                "late_uncertainty_gate": 0.0,
                "max_rival_rounds": 1,
                "max_repair_children": 0,
                "max_late_repairs": 0,
                "noise_policy": "fresh_noise",
                "strength": 1.0,
                "late_noise_policy": "fresh_noise",
                "late_strength": 1.0,
                "tail_top_time": 1.0,
                "tail_rival_time": 0.0,
                "stop_gap": 0.020,
        ...
        if beta <= 0.75:
            return {
                ...
                "rival_gap": 0.020,
                "repair_gap": 0.010,
                "late_repair_gap": 0.014,
                "uncertainty_gate": 0.17,
                "late_uncertainty_gate": 0.10,
                "max_rival_rounds": 2,
                "max_repair_children": 1,
                "max_late_repairs": 1,
                "noise_policy": "fresh_noise",
                "strength": 0.85,
                "late_noise_policy": "mixed_noise",
                "late_strength": 0.45,
                "tail_top_time": 1.0,
                "tail_rival_time": 1.0,
                "stop_gap": 0.015,
        ...
        return {
            ...
            "rival_gap": 0.017,
            "repair_gap": 0.009,
            "late_repair_gap": 0.012,
            "uncertainty_gate": 0.16,
            "late_uncertainty_gate": 0.08,
            "max_rival_rounds": 3,
            "max_repair_children": 1,
            "max_late_repairs": 1,
            "noise_policy": "fresh_noise",
    ...
    def _late_repair_stage(
        ...
        if int(schedule["max_late_repairs"]) <= 0:
            return particle_ids
        if self._spent(env, initial_budget) >= int(schedule["target_nfe"]):
            return particle_ids

        repairs = 0
        active_ids = list(particle_ids)
        while repairs < int(schedule["max_late_repairs"]):
            previews = self._rank_previews(env, active_ids)
            if len(previews) < 2:
                return active_ids
            ...
            gap = float(best.score or 0.0) - float(runner_up.score or 0.0)
            uncertainty = max(float(best.uncertainty or 0.0), float(runner_up.uncertainty or 0.0))
            if gap > float(schedule["late_repair_gap"]) and uncertainty < float(schedule["late_uncertainty_gate"]):
                return active_ids

            pair = previews[:2]
            if max(float(preview.time) for preview in pair) < float(schedule["rival_time"]) - 1e-9:
                return active_ids
            if self._spent(env, initial_budget) < int(schedule["stop_spend_floor"]) - 2:
                return active_ids
            ...
            anchor = self._late_repair_anchor(best, runner_up)
            try:
                child_ids = env.backward(
                    anchor.id,
                    target_time=float(schedule["late_repair_time"]),
                    noise_policy=str(schedule["late_noise_policy"]),
                    num_children=1,
                    strength=float(schedule["late_strength"]),
                )
            except (BudgetExceededError, InvalidActionError):
                return active_ids
        ...
        while True:
            ...
            if len(previews) >= 2:
                ...
                if (
                    ...
                    and spent >= int(schedule["min_stop_spend"])
    ...
    def _late_repair_anchor(self, best: PreviewRecord, runner_up: PreviewRecord) -> PreviewRecord:
        best_unc = float(best.uncertainty or 0.0)
        rival_unc = float(runner_up.uncertainty or 0.0)
        if rival_unc >= best_unc - 1e-9:
            return runner_up
        if float(best.time) < float(runner_up.time) - 1e-9:
            return best
        return runner_up
