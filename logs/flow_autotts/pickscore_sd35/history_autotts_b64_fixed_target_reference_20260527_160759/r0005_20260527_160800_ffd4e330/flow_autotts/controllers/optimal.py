...
class OptimalController:
    """Beta-scheduled scout-commit controller with selective dual-anchor repair."""
    ...
    def _single_root_preview(
        ...
        try:
            ...
            while env.nfe_used < target_nfe and env.budget_left > 0:
                self._preview(env, root_id)
        except BudgetExceededError:
            return self._safe_answer(env)
        except InvalidActionError:
    ...
    def _repair_stage(
        ...
        anchor_candidates = [best, runner_up]
        ...
        while True:
            ...
            if len(previews) >= 2:
                ...
                if (
                    ...
                    return
                if (
                    gap <= float(schedule["fill_gap"])
                    or uncertainty >= float(schedule["fill_uncertainty_gate"])
                ):
                    confirm_ids = [preview.particle_id for preview in previews[: int(schedule["fill_confirmation_cap"])]]
                    progressed = False
    ...
    def _forward_to(
        ...
        particle = state.particles.get(particle_id)
        if particle is None or particle.status != "active":
            raise InvalidActionError(f"particle {particle_id} is not active")
        for time_value in env.time_grid:
    ...
    def _prune_non_survivors(self, env: FlowTTSEnv, particle_ids: list[int], survivors: list[int]) -> None:
    ...
    def _finish_one(self, env: FlowTTSEnv, particle_id: int | None) -> AnswerRecord:
        ...
        try:
            ...
            return self._safe_answer(env, fallback_rule="latest_active")
        return self._safe_answer(env)

    def _safe_answer(self, env: FlowTTSEnv, fallback_rule: str = "best_preview_score") -> AnswerRecord:
        try:
            return env.answer(rule=fallback_rule)
        except (BudgetExceededError, InvalidActionError):
    ...
    def _can_spend(
        ...
        target_nfe = min(int(schedule["target_nfe"]), int(env.budget), initial_budget)
        return self._spent(env, initial_budget) + int(extra_cost) <= target_nfe and env.budget_left >= int(extra_cost)

    def _spent(self, env: FlowTTSEnv, initial_budget: int) -> int:
        return max(0, int(initial_budget - env.budget_left))

    def _sde_cfg(self, schedule: dict[str, float | int | str]) -> dict[str, float]:
        return {
